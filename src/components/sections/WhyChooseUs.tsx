      ...
      <ul className="mt-14 grid gap-x-12 gap-y-8 md:grid-cols-2">
        {REASONS.map((reason) => (
          <li key={reason.title} className="flex gap-4 border-t border-line pt-6">
            <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-pill border border-accent/40 bg-accent/25 shadow-xs">
              <Check size={14} className="stroke-[2.5] text-ink" aria-hidden="true" />
            </span>
            <div>
